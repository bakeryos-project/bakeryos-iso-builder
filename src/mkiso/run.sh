...
# Original at: https://gitlab.archlinux.org/archlinux/archiso
# Modified by smtdfc for BakeryOS
...
usage: bakeryos-mkiso [options] <profile_dir>
  options:
...
_make_customize_airootfs() {
    ...
    if [[ -d "${profile}/hooks" ]]; then
        ...
        for _hook in "${profile}/hooks/"*.sh; do
            [[ -e "${_hook}" ]] || continue
            _hook_name="${_hook##*/}"
            cp -a -- "${_hook}" "${pacstrap_dir}/tmp/hooks/${_hook_name}"
            chmod -f -- +x "${pacstrap_dir}/tmp/hooks/${_hook_name}"
            _msg_info "Running hook: ${_hook_name}"
            eval -- env -u TMPDIR chroot "${pacstrap_dir}" "/tmp/hooks/${_hook_name}"
        done
        rm -rf -- "${pacstrap_dir}/tmp/hooks"
        _msg_info "Done! Hooks run successfully."
    fi
...
_make_common_bootmode_uefi.systemd-boot_copy_files() {
    ...
    if [[ -e "${pacstrap_dir}/usr/share/edk2-shell/${target_arch,,}/Shell_Full.efi" ]]; then
        mcopy -i "${efibootimg}" \
            "${pacstrap_dir}/usr/share/edk2-shell/${target_arch,,}/Shell_Full.efi" "::/shell${target_arch,,}.efi"
